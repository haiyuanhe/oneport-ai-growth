...
import { 
  Package, Search, BarChart3, Target, DollarSign, 
  ShoppingCart, Users, MessageSquare, Truck, Calculator,
  BookOpen, Check
} from "lucide-react";
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative py-24 overflow-hidden">
        <div className="absolute inset-0">
          <img 
            src={amazonSolutionsImage} 
            alt="Amazon Solutions" 
              ...
              <div key={categoryIndex}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {category.items.map((item, itemIndex) => (
                    <Card key={itemIndex} className="hover:shadow-lg transition-all duration-300 group">
                      <CardHeader>
          ...
          <div className="max-w-md mx-auto">
            <Card className="relative overflow-hidden">
              ...
              <CardContent className="space-y-4">
                {(t('amazon.pricing.basic.features', { returnObjects: true }) as string[]).map((feature: string, index: number) => (
                  <div key={index} className="flex items-center space-x-3">
                    <Check className="h-5 w-5 text-green-500 flex-shrink-0" />
                    <span className="text-sm">{feature}</span>
                  </div>
                ))}
                <Button className="w-full mt-6 bg-gradient-to-r from-[hsl(var(--oneport-orange))] to-[hsl(var(--oneport-orange-dark))] hover:opacity-90">
                  Start Free Trial
                </Button>
              </CardContent>
            </Card>
